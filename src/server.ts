...
const provider = config.APPYPAY_MODE === 'mock' ? new MockAppyPayProvider() : new RealAppyPayProvider(config);
const oauth = config.SHOPIFY_AUTH_ENABLED ? new ShopifyOauthService(config, prisma) : undefined;
const app = buildApp(config, new PaymentService(provider, new PrismaPaymentStore(prisma)), oauth);
